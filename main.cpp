...
int main() {
	...
	SkeletalAnimation coach_dance("models/coach/Clapping.dae", &coach_model);
	SkeletalAnimator coach_animator(&coach_dance);

	auto& coach = coach_model.getRoot();
	coach.grow(glm::vec3(1.2, 1.2, 1.2));
	coach.move(glm::vec3(3, 0, -5));
	//coach.addTexture(loadTexture("models/kid/textures/kid_specular.png", "normalMap"));

		// goalkeeper 
	Skeletal goalkeeper_model("models/goalkeeper/goalkeeper.dae", true);

	SkeletalAnimation goalkeeper_dance("models/goalkeeper/goalkeeper.dae", &goalkeeper_model);
	SkeletalAnimator goalkeeper_animator(&goalkeeper_dance);

	auto& goalkeeper = goalkeeper_model.getRoot();
	goalkeeper.grow(glm::vec3(1.2, 1.2, 1.2));
	goalkeeper.move(glm::vec3(0, 0, -6));
	//goalkeeper.addTexture(loadTexture("models/goalkeeper/textures/goalkeeper_specular.png", "normalMap"));


	// kid 
	Skeletal skeletal_model("models/kid/kid.dae", true);

	SkeletalAnimation dance_animation("models/kid/kid.dae", &skeletal_model);
	SkeletalAnimator skeletal_animator(&dance_animation);

	SkeletalAnimation idle_animation("models/kid/idle.dae", &skeletal_model);
	SkeletalAnimator idle_animator(&idle_animation);

	auto& kid = skeletal_model.getRoot();
	...
	while (running) {
		...
		if (moving && kid.getPosition().y == 0) {
			...
			kid_transforms = skeletal_animator.GetFinalBoneMatrices();
		}
		else {
